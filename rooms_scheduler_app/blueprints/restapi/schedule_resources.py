...
class SchedulesResource(Resource):
    ...
    def post(self):
        ...
        if check_room_availability(
                start_time=start_time, end_time=end_time, date=date, room_id=room_id):
            pass
...
class ScheduleResource(Resource):
    ...
    def get(self, schedule_id):
        schedule = Schedule.query.get(schedule_id) or abort(
    ...
    def patch(self, schedule_id):
        ...
        # Checking if the selected period is available
        if check_room_availability(
                start_time=schedule.start_time, end_time=schedule.end_time, date=schedule.date,
                room_id=room_id):
            pass
...
class AccessRequestHandlerResource(Resource):
    """ Handler for access of a room.
        ...
        Verifies if the user is allowed to access the room and
        if he has scheduled the room at the moment of request.
    ...
    def get(self, user_id, room_id):
        ...
        for schedule in schedules:
            ...
            if now > schedule.start_time and now < schedule.end_time and (
